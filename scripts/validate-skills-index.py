...
def check_item(item, index: int, seen_ids: set) -> None:
    ...
    all_only = item.get("allOnly")
    if all_only is not None and not isinstance(all_only, bool):
        err(f"{label}: 'allOnly' when present must be a boolean, got {all_only!r}")
...
def check_attachments_have_exit(items: list) -> None:
    ...
    were named in `iot-product-workflow`, a skill that agent never
