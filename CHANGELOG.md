...
### CLI-first skills catalogue and field-tested miniapp guidance (2026-09-07)

- Consolidated the skills catalogue into the current `skills/<group>/<id>/`
  layout with one `skills/index.json` and four install groups: `core`,
  `embedded`, `cloud`, and `miniapp`. The index currently contains 24 items;
  the index is authoritative for counts and SDK applicability.
- Added the CLI relationship declarations, group/install-payload checks,
  domain-version validation, and the current `tuyaopen-start` routing/reference
  structure used by the IDE and standalone CLI.
- Corrected field-tested guidance: custom `qt` DPs use IDs 101–199,
  `SmartDeviceModel.init()` is required before relying on `SdmProvider`,
  device authorization includes the free-code and purchase routes, and miniapp
  review normally takes about two minutes before release.
- Kept the Waveshare T5AI QMI8658 board metadata from the upstream main version;
  this PR does not duplicate the board fix already delivered in #70.
- Marked `AIDentalMirrorTemplate`, `PanelLock`, and `GyroSweeperTemplate`
  unpublished in `miniapp-templates/miniapp-template-covers.json` pending
  further validation.
- Updated the README and changelog to describe the current 24-item, four-group
  catalogue without hard-coding stale historical counts.
- Kept `release.json` at the official release metadata; domain versions are
  used for content refresh signalling.


### Current catalogue alignment and template visibility (2026-09-07)

- Kept the official QMI8658 metadata already present for
  `waveshare-t5ai-touch-amoled-175` instead of duplicating the upstream board
  fix in this PR.
- Marked `AIDentalMirrorTemplate`, `PanelLock`, and `GyroSweeperTemplate` as
  `publish: false` while those panels remain under validation.
- Updated the repository README and this changelog to describe the current
  four install groups and the current skill index as the source of truth for
  item counts and SDK applicability.
...
的事一个字都没有 ——「同一个码同一时间只能用在一台设备上」检索为 0 命中,而「码从哪来」只有一行
`license import --xlsx`(也就是「你已经有一个 Excel 了」)。

**规则一:可反复烧写、可擦除。** 这一条其实已经体现在风险分级里(`firmware authorize`
2026-08-18 从 P0 降到 P2,因为写的是一条可重写的 KV 且 `auth-status` 能读回来),但技能没把它
和「所以写错了不要紧」连起来说。

**规则二:同一个码同时只能给一台设备。** 这条从命令行为里看不出来,却决定了用法 —— 把同一个
UUID 写进第二台设备不是「两台都能用」,而是两台冲突(云端按 UUID 认设备)。所以第二台设备要联网
就得**再要一个码**;而一台设备**换**码是安全的,把旧设备的码挪到新设备也可以。

**因此写码前必须问用户**,而且不是走确认门那种形式确认,是真的要问清「这个码现在是不是空闲」。
技能里给了一句可直接照用的问法。

**没有码时怎么办**:新增一节。`tuyaopen` 侧**没有申领命令** —— `license` 组四条全是本地库操作,
云端申领只在 IDE(内部走 `tuya-devplat-cli auth-code fetch --pid`)或平台网页。列了三条获取路径
及各自适用场景,并明确要求拿不到码时**停下来告诉用户卡在哪**,不要继续往下做让设备卡在
`client no active`,也不要编一个 UUID 试。

`manifests.skills.version` 1.5.0 → **1.6.0**。
