...
description: >-
  Which `tuyaopen` command group covers an embedded-side intent, and the
  ...
  task steps and no flag lists: flags come from `tuyaopen schema get`, and how
...
The embedded side of the `tuyaopen` CLI is spread over more command groups
...
|---|---|---|
| Flags, defaults, argument shapes | `tuyaopen schema get --group <g> --command <c>` | A hand-written flag list rots. This catalogue has been burned by transcribed numbers four separate times; `schema get` is generated from the command definitions and cannot drift |
...
|---|---|---|
| Set up or activate the SDK environment; clone or update the SDK | `tuyaopen sdk clone` · `sdk env-init` · `sdk env-pull` · `sdk update` | `tuyaopen-embedded-env-setup` |
| Create a project; change platform / board; bind a product; read project metadata | `tuyaopen project info` · `project create` · `project set-platform` · `project set-board` · `project bind-product` | `tuyaopen-embedded-project` |
| Browse the demo / example catalogue | `tuyaopen demos list` · `demos detail` | `tuyaopen-embedded-project` |
| See which boards exist and what they carry | `tuyaopen boards list` · `boards detail` | `tuyaopen-embedded-hardware` |
| Compile; clean the build tree | `tuyaopen firmware build` · `firmware clean` | `tuyaopen-embedded-build` |
| Flash firmware; watch serial output; find the port | `tuyaopen firmware flash` · `firmware monitor` · `tuyaopen firmware list-ports` | `tuyaopen-embedded-flash` |
| Use a peripheral — display, camera, button, LED, I2C, UART, anything with a pin | `tuyaopen hardware list-used` · `hardware board-context` · `hardware set-used` | `tuyaopen-embedded-hardware` |
| Get an authorization code onto the device (UUID / AuthKey / PID) | `tuyaopen license list` · `license add` · `license import` · `tuyaopen firmware authorize` · `firmware auth-status` | `tuyaopen-embedded-device-auth` |
| See what the **SDK itself** is made of — core + platform sub-SDKs, installed? current? | `tuyaopen sdk platform` | `tuyaopen-embedded-env-setup` |
| Get a **third-party** library into the project — find it, install it, record it | `tuyaopen dependency search` · `dependency install` · `dependency add` · `dependency list` · `dependency remove` | `tuyaopen-embedded-dependency` (opt-in — see below) |
| Find out why the environment or the CLI itself is wrong; produce a bug-report bundle | `tuyaopen diag doctor` · `diag export` | `tuyaopen-shared` |
| Drive the device over its serial CLI; capture logs in the background; decode a crash dump | `tuyaopen firmware monitor` · `tuyaopen firmware list-ports` | `tuyaopen-embedded-cli-debug` |
...
| 你要的 | 用 | 是什么 |
|---|---|---|
| **涂鸦给你的** —— SDK 本体和平台子 SDK 装了没、是不是最新 | `tuyaopen sdk platform` | git 仓库,按 commit 固定,住在 workspace 的 SDK 目录下。就是 IDE 里「库 → TuyaOpen」那一栏 |
| **别人的** —— 找一个 PlatformIO 库、装进本工程、记进锁文件 | `tuyaopen dependency …` | tarball,住在工程里,版本写进 `.tuyaopen/dependencies.lock.json`。就是 IDE 里「库 → 生态」那一栏 |
...
```bash
tuyaopen skills install --ids tuyaopen-embedded-dependency
```
...
1. **The SDK environment must be active** before anything except `sdk` and
   ...
   environment, which sends readers looking in the wrong place. `tuyaopen diag
...
3. **Platform and board must be set before most commands mean anything.** A
   ...
   flash, or answer a hardware question. `tuyaopen project info` shows both.
...
Some embedded work still has no `tuyaopen` command and goes through the SDK's
