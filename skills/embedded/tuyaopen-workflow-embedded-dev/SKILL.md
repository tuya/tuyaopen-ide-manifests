...
```
① iot-product-workflow            requirements → product/PID → DPs → dp generate
                │
                ├──▶ ② tuyaopen-workflow-embedded-dev   ← you are here
                └──▶ ③ miniapp-development-workflow     (independent of ②)
```
...
|---|---|
| Handed over from `iot-product-workflow` | Nothing to re-negotiate — read the context files listed below and enter at the state it reported (`has-dps`, `in-progress` or `built`) |
...
machines that must be kept in sync. `iot-product-workflow` § *State
