...
`iot-product-workflow` — go back there rather than improvising.

| Trigger | Action |
|---------|--------|
| Build fails: missing Kconfig | → `has-dps` Step 5 |
| Build fails: anything else | Stay in `in-progress`. Debug with the loop below |
| A DP has no handler | → `in-progress` Step 1 (gap analysis). If the DP does not exist on the platform at all, that is phase ① — go back to `iot-product-workflow` |
