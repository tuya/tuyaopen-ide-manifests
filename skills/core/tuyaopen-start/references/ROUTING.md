...
| Skill id | Use when |
|---|---|
| `iot-product-workflow` | **Start here for "I want to build a product".** Platform phase — requirements → product/PID → DPs → `dp generate`. A state machine that resumes from wherever the project stands. Hands the PID + DPs + generated header to the next two |
...
(The panel *workflow* entry point is `miniapp-development-workflow`, above.)
