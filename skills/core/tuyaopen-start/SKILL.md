...
- Devplat writes take `--dry-run` → `--confirm <token>` rather than `--yes`,
  and `product solution-list` always returns empty — `custom-list` is the
  real listing call. The wrong recipe — `solution-list` + `--yes` — had
  already been sent when the reference stating this was finally read.
- `tuyaopen-start` § 8 draws the project layout (`.tuyaopen/` +
  `source/embedded/`). It was instead re-derived from a `firmware build` error
  and rebuilt by hand.

`iot-product-workflow` was installed and never opened. It is the
