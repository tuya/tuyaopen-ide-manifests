...
├── cloud/                         # 2 payloads
│   └── tuyaopen-cloud/  iot-product-workflow/
└── miniapp/                       # 7 payloads
    ├── tuyaopen-miniapp/  miniapp-development-workflow/  tuyaopen-miniapp-ray-common/
...
`iot-product-workflow` is `surface: "embedded"` with `group: "cloud"`.
...
`iot-product-workflow`). Same word, two vocabularies — only the
...
`miniapp-development-workflow`, keeping the old name as an `aliases` entry.)
