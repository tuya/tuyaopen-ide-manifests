#!/usr/bin/env node
/**
 * miniapp-development-workflow/validate.mjs
 *
...
async function main() {
  ...
  if (!root) {
    console.error(`\x1b[31m[miniapp-development-workflow] cannot find project.tuya.json starting from ${start}\x1b[0m`);
  ...
  console.log(`[miniapp-development-workflow] validating: ${root}\n`);
...
main().catch((e) => {
  console.error(`\x1b[31m[miniapp-development-workflow] crash: ${e.stack ?? e.message ?? e}\x1b[0m`);
  process.exit(4);
});
