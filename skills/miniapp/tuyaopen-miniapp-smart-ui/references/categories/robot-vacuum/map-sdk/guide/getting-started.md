...
::: tip 响应式配置
配置是**响应式**的，当你修改传入的 `config` prop 时，地图会自动更新视觉效果。这使得主题切换、颜色调整等功能变得简单易实现。详见[配置](../reference/config.md)。
:::

查看[配置](../reference/config.md)了解更多配置项。
...
查看[运行时](../reference/runtime.md)了解更多配置。
...
查看[地图事件回调](../reference/callbacks.md)了解更多事件回调。
...
查看[地图方法](../reference/methods.md)了解更多 API 。
